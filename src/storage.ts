...
export async function populateFromManagedStorage(): Promise<void> {
  const managedStorageItems = chrome.storage.managed.get([
    'khulnasoftPortalUrl',
    'khulnasoftEnterpriseDefaultModel',
    'khulnasoftAllowlist',
  ]);
  void managedStorageItems.then((result) => {
    if (result.portalUrl !== undefined) {
      void setStorageItem('portalUrl', result.khulnasoftPortalUrl);
    }
    if (result.enterpriseDefaultModel !== undefined) {
      void setStorageItem('enterpriseDefaultModel', result.khulnasoftEnterpriseDefaultModel);
    }
    if (result.allowlist !== undefined) {
      const lst = result.khulnasoftAllowlist
        .split('\n')
        .map((x: string) => x.trim())
        .filter((x: string) => x !== '');
